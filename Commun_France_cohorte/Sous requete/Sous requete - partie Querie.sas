...
proc sql noprint;
create table OPEN_MESSAGE AS
SELECT MIMESSAGEHISTORY,MIMESSAGEOBJECTID, MIMESSAGETYPE, MIMESSAGESTATUS ,MIMESSAGEpersonid as personid, MIMESSAGESITE as trialsite, MIMESSAGEVISITID as visitid,MIMESSAGECRFPAGEID as CRFPAGEID, MIMESSAGEVISITCYCLE as VISITCYCLEnumber,MIMESSAGECRFPAGECYCLE as CRFPAGECYCLEnumber,MIMESSAGEDATAITEMID as dataitemid, MIMESSAGE.MIMESSAGERESPONSECYCLE as repeatnumber,
MIMESSAGERESPONSEVALUE as OPEN_value "OPEN_value" , MIMESSAGE.MIMESSAGETEXT as OPEN_txt "OPEN_txt", int(MIMESSAGECREATED)-21916 AS OPEN_dt format=ddmmyy10.
FROM &stu..MIMESSAGE where MIMESSAGETYPE=3 and MIMESSAGESTATUS=1;

create table closed_MESSAGE AS
SELECT MIMESSAGEHISTORY,MIMESSAGEOBJECTID, MIMESSAGETYPE, MIMESSAGESTATUS ,MIMESSAGEpersonid as personid, MIMESSAGESITE as trialsite, MIMESSAGEVISITID as visitid,MIMESSAGECRFPAGEID as CRFPAGEID, MIMESSAGEVISITCYCLE as VISITCYCLEnumber,MIMESSAGECRFPAGECYCLE as CRFPAGECYCLEnumber,MIMESSAGEDATAITEMID as dataitemid, MIMESSAGE.MIMESSAGERESPONSECYCLE as repeatnumber,
MIMESSAGERESPONSEVALUE as closed_value"closed_value" , MIMESSAGE.MIMESSAGETEXT as closed_txt "closed_txt", int(MIMESSAGECREATED)-21916 AS closed_dt format=ddmmyy10.
FROM &stu..MIMESSAGE where MIMESSAGETYPE=3 and MIMESSAGESTATUS=2;



create table all_temp1 as select 

 * from OPEN_MESSAGE a left join closed_MESSAGE b on a.trialsite=b.trialsite and a.personid=b.personid and a.VISITID=b.visitid and
...
proc sql noprint; create table all_temp2 as select  a.*,localidentifier1 from all_temp1 a left join &stu..trialsubject  b on a.personid=b.personid and a.trialsite=b.trialsite;

proc sql noprint; create table all_temp3 as select  a.*,crftitle,crfpageorder from all_temp2 a left join &stu..crfpage  b on a.crfpageid=b.crfpageid;
proc sql noprint; create table all_temp5 as select  a.*,VISITORDER,visitname,REPEATING from all_temp3 a left join &stu..studyvisit  b on a.visitid=b.visitid;
proc sql noprint; create table all_temp6 as select  a.*,dataitemname from all_temp5 a left join &stu..dataitem  b on a.dataitemid=b.dataitemid;


data all_&stu;
retain  LOCALIDENTIFIER1 VISITNAME VISITCYCLENUMBER CRFTITLE CRFPAGEINSTANCELABEL;
set all_temp6;
run;
...
%relance(stu=incl,ora=ora_stui);
%relance(stu=suiv,ora=ora_stus);
...
data incl.finaldcr;set incl.finaldcr ;format base base.; base=1;run;
data suiv.finaldcr;set suiv.finaldcr ;format base base.; base=2;run;


data All_incl;set All_incl ;format base base.; base=1;run;
data All_suiv;set All_suiv ;format base base.; base=2;run;
data all_message;set All_incl All_suiv;run;


data finaldcr;
set incl.finaldcr suiv.finaldcr;
if LOCALIDENTIFIER1 NE "" and trialsite ne "supprime";
run;
proc sort; by LOCALIDENTIFIER1 base visitorder visitcyclenumber crfpageorder crfpagecyclenumber repeatnumber;run;
